...
export default function App() {
  ...
  if (loading) return (
    <div style={{
      ...
      <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: -1 }}>틈</div>
...
function WorkspaceWithData({ user, profile, onSignOut, onUpdateProfile }) {
  ...
  // 팀채팅 / DM은 선택된 채널에 따라 동적으로 구독
  const [selDM, setSelDM] = useState(undefined)
  const chatChannel = selDM === undefined ? null : selDM === null ? null : selDM
  const { messages: teamMessages, sendMessage: sendTeamMessage } = useMessages(null)
  const { messages: dmMessages, sendMessage: sendDMMessage } = useMessages(
    selDM && selDM !== null ? [user.id, selDM].sort().join('_') : null
  )
