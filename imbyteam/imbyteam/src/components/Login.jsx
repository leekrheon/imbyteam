...
const CSS = `
  ...
  .name-card-title { font-size: 18px; font-weight: 800; color: #111; margin-bottom: 6px; }
  .name-card-sub { font-size: 13px; color: #888; margin-bottom: 24px; line-height: 1.5; }
`
...
export default function Login({ onSignIn, onSignUp }) {
  ...
  async function handleSubmit(e) {
    ...
    if (mode === 'signin') {
      ...
      if (!name.trim() || !ini.trim()) {
        setError('이름과 이니셜을 입력해주세요.')
        setLoading(false)
        return
      }
      const err = await onSignUp(email, password, name.trim(), ini.trim().slice(0, 1), role)
    ...
    <>
      ...
      <div className="login-root">
        <div className="login-blobs">
          ...
          <div className="login-blob b3" /><div className="login-blob b4" />
        </div>
        ...
        <div className="login-card">
          ...
          <div className="login-tabs">
            {['signin', 'signup'].map(m => (
              <button key={m} type="button" className={`login-tab${mode === m ? ' on' : ''}`}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <>
                <div style={{ display: 'flex', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input className="login-input" placeholder="홍길동" value={name}
                      onChange={e => setName(e.target.value)} />
                  </div>
                  <div style={{ width: 80 }}>
                    <label className="login-label">이니셜</label>
                    <input className="login-input login-ini-input" placeholder="길" maxLength={1}
                      value={ini} onChange={e => setIni(e.target.value)} />
                  </div>
                </div>
                <div>
                  <label className="login-label">직책/역할</label>
                  <input className="login-input" placeholder="크리에이티브 디렉터" value={role}
            ...
            <div>
              ...
              <input className="login-input" type="email" placeholder="hello@imby.kr"
            ...
            <div>
              ...
              <input className="login-input" type="password"
                ...
                value={password} onChange={e => setPassword(e.target.value)} required minLength={mode === 'signup' ? 6 : undefined} />
            </div>
            {error && <div className="login-error">{error}</div>}
            <button type="submit" disabled={loading} className="login-submit">
              {loading ? (mode === 'signin' ? '로그인 중...' : '가입 중...') : (mode === 'signin' ? '로그인' : '가입하기')}
            </button>
          </form>
        </div>
      </div>
...
/* ── 처음 로그인 시 이름 입력 팝업 ── */
export function NameSetupModal({ onSubmit }) {
  const [name, setName] = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  ...
  return (
    <>
      <style>{CSS}</style>
      <div className="name-scrim">
        <div className="name-card">
          <div className="name-card-title">안녕하세요! 👋</div>
          <div className="name-card-sub">팀에서 어떻게 불릴지 이름을 알려주세요.<br />언제든지 프로필에서 변경할 수 있어요.</div>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label className="login-label">이름</label>
              <input className="login-input" placeholder="홍길동" value={name}
                onChange={e => setName(e.target.value)} autoFocus maxLength={20} />
            </div>
            {error && <div className="login-error">{error}</div>}
            <button type="submit" disabled={!ok || loading} className="login-submit"
              style={{ opacity: ok && !loading ? 1 : 0.5 }}>
              {loading ? '저장 중...' : '시작하기'}
            </button>
          </form>
        </div>
