...
export default function Login({ onSignIn, onSignUp }) {
  ...
  return (
    <>
      <style>{`
        @keyframes login-swirl-1 { 0%,100%{transform:translate(0,0) scale(1)} 33%{transform:translate(12%,18%) scale(1.15)} 66%{transform:translate(-8%,8%) scale(.95)} }
        @keyframes login-swirl-2 { 0%,100%{transform:translate(0,0) scale(1)} 33%{transform:translate(-15%,10%) scale(1.1)} 66%{transform:translate(10%,-12%) scale(.9)} }
        @keyframes login-swirl-3 { 0%,100%{transform:translate(0,0) scale(1)} 50%{transform:translate(8%,-15%) scale(1.12)} }
        @keyframes login-swirl-4 { 0%,100%{transform:translate(0,0) scale(1)} 50%{transform:translate(-12%,14%) scale(1.08)} }
        .login-root {
        ...
        .login-blob { position: absolute; border-radius: 50%; mix-blend-mode: multiply; }
        .login-blob.b1 { top: -20%; left: -15%; width: 60%; padding-bottom: 60%; background: #22cd6d; filter: blur(80px); opacity: .65; animation: login-swirl-1 12s infinite ease-in-out; }
        .login-blob.b2 { top: -10%; right: -20%; width: 55%; padding-bottom: 55%; background: #38d98a; filter: blur(75px); opacity: .45; animation: login-swirl-2 15s infinite ease-in-out; }
        .login-blob.b3 { bottom: -25%; left: 5%; width: 50%; padding-bottom: 50%; background: #a3e635; filter: blur(85px); opacity: .4; animation: login-swirl-3 13s infinite ease-in-out; }
        .login-blob.b4 { bottom: -15%; right: 10%; width: 45%; padding-bottom: 45%; background: #5eead4; filter: blur(80px); opacity: .35; animation: login-swirl-4 17s infinite reverse ease-in-out; }
        .login-glass {
        ...
        @media (prefers-color-scheme: dark) {
          .login-blob.b1, .login-blob.b2, .login-blob.b3, .login-blob.b4 { opacity: .25; }
          .login-glass { background: rgba(20,20,22,.55); }
          .login-card { background: rgba(28,30,34,.75); border-color: rgba(255,255,255,.10); box-shadow: 0 12px 48px rgba(0,0,0,.3); }
          .login-logo svg path { fill: #eee !important; }
          .login-tabs { background: rgba(255,255,255,.06); }
          .login-tab.on { background: rgba(255,255,255,.12); color: #eee; box-shadow: none; }
          .login-tab:not(.on) { color: #777; }
          .login-label { color: #999; }
          .login-input { background: rgba(255,255,255,.06); border-color: rgba(255,255,255,.10); color: #eee; }
          .login-input:focus { background: rgba(255,255,255,.10); border-color: #05d560; }
          .login-input::placeholder { color: #666; }
          .login-submit { background: #eee; color: #111; }
        }
      `}</style>
