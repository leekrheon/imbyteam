...
function WorkspaceWithData({ user, profile, onSignOut, onUpdateProfile }) {
  ...
  // DM 채널 — ini 기반 (profiles 비동기 로드에 의존하지 않음)
  // 양측에서 동일하게 계산: dm_소_주 (sort로 통일)
  const dmChannel = (selDM && selDM !== null && myIni)
    ? `dm_${[myIni, selDM].sort().join('_')}`
    : undefined
  ...
  async function handleSendDM(text, refData) {
    if (selDM === null) {
      ...
      // 상대방에게 DM 알림 (profiles에서 UUID 조회)
      const partnerProfile = profiles.find(p => p.ini === selDM)
      if (partnerProfile) {
